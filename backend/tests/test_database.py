...
def test_deleting_chat_cascades_only_its_messages(tmp_path: Path) -> None:
    database_path = tmp_path / "data" / "chat.db"
    database_url = f"sqlite:///{database_path}"
    ...
    try:
        default_chat = connection.execute(
            """
            INSERT INTO chats (
                study_date,
                kind,
        ...
        extra_chat = connection.execute(
            ...
            (
                ...
                "extra",
                1,
                "2026-09-07 영어 학습 - 추가 학습 (1)",
                "2026-09-07T10:00:00+09:00",
            ),
        )

        default_chat_id = default_chat.lastrowid
        extra_chat_id = extra_chat.lastrowid

        assert default_chat_id is not None
        assert extra_chat_id is not None
        ...
        connection.execute(
            ...
            (
                ...
                "추가 학습 메시지",
                1,
                "2026-09-07T10:01:00+09:00",
            ),
        )
        connection.commit()

        connection.execute("DELETE FROM chats WHERE id = ?", (default_chat_id,))
        connection.commit()
    ...
    assert remaining_messages == [(extra_chat_id,)]
