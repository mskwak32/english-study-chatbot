...
- 확인 절차가 포함된 채팅 삭제
- 로딩, 오류, 빈 화면 상태
- 오늘 기본 학습이 없을 때 오른쪽 메인 영역 전체에서 명시적으로 시작하고 삭제 후 자동 생성하지 않는 흐름
- 내용 높이에 맞는 메시지 영역과 데스크톱·모바일 키보드 전송
- 모바일 반응형 레이아웃
- 학습 프로필, 최근 학습 이력, 복습 단어를 조회하는 읽기 전용 API
- Topbar 아이콘에서 진입하고 세 정보를 탭으로 전환하는 단일 학습 정보 페이지
...
- FastAPI가 같은 출처에서 정적 Web UI와 채팅·학습 정보 API를 제공하도록 구성했다. 브라우저는 상대 경로 API만 호출하며 Ollama에는 직접 연결하지 않는다.
- 오늘 기본 학습은 조회와 시작을 분리했다. 기록이 없을 때는 오른쪽 메인 영역 전체에 시작 화면만 보이고, 시작·삭제·새로고침 뒤의 상태 전환을 확인했다.
- 학습 프로필이 없을 때는 일반 학습을 시작하지 않고, 초기 실력 테스트의 목적·14문항 구성을 안내하는 프로필 생성 화면을 표시한다. 초기 테스트 완료 도구는 점수와 레벨을 서버 기준으로 다시 검증해 프로필과 테스트 결과를 저장한다.
- 날짜별 채팅 선택, 추가 학습 생성, 삭제 확인, 메시지 전송 대기·오류, Cmd/Ctrl+Enter 및 모바일 Enter 전송을 구현했다.
- 학습 프로필, 최근 학습 이력 20건, 복습 단어를 Topbar 아이콘의 단일 탭형 페이지에서 조회할 수 있다.
- 360px·768px·1280px 너비의 반응형 레이아웃과 브라우저 핵심 흐름을 확인했다.
- 로컬 Docker 이미지에서 호스트 Ollama(`host.docker.internal:11434`)와 연결하고 호스트 포트 `8100`을 통해 대화·SQLite 복원을 확인했다.
- 완료 커밋 직전에 Ruff 자동 수정·검사, 전체 Python 테스트 86개, Web 테스트 10개를 실행했다. 이후 초기 실력 테스트·프로필 생성 흐름을 추가한 뒤 전체 Python 테스트 91개와 Web 테스트 10개를 실행했다. Python 테스트의 외부 의존성 폐기 경고 2개 외에는 실패가 없었다.
