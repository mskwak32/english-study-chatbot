...
Raspberry Pi 5에서 네이티브 Ollama와 Python Agent를 실행하고 PC·휴대폰 브라우저에서 사용하는 개인용 영어 학습 챗봇을 만든다.
...
- Agent는 LAN의 `8100` 포트를 사용하고, Ollama는 Pi 로컬 `127.0.0.1:11434`에서만 실행한다.
- Pi의 `.env`, `data/`, `backups/`, Ollama 모델은 release 밖의 영속 경로에 두며 배포·롤백에서 변경하지 않는다.
...
| 6. 네이티브 Pi 운영 기반 | 진행 중 | systemd Agent, 로컬 Ollama, 영속 경로 분리 |
| 7. PC 기반 rsync 배포 | 진행 중 | 테스트 후 release 전송, Agent-only 전환·롤백 |
...
- Ollama는 Pi의 systemd 서비스로 `127.0.0.1:11434`에서 실행하고 필요한 모델을 미리 준비한다.
- Agent는 systemd 서비스로 `8100` 포트에서 실행하며 Pi `.env`의 설정과 영속 SQLite 경로를 사용한다.
- 개발 PC의 한 배포 명령은 Python·Web 테스트, commit별 `rsync` release 전송, Agent 재시작, `/health` 검사를 수행한다.
- 개발 PC의 한 롤백 명령은 직전 release로 `current` 링크를 바꾸고 Agent만 재시작한다.
- 최신 배포·직전 release 롤백·최신 release 재배포 동안 SQLite DB·백업·`.env`·Ollama 모델이 유지되는 것을 검증한다.
